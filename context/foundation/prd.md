...
poprawić". Efektem jest opóźniona reakcja i działania firmy.

Status quo (arkusze, intuicja, ewentualnie raporty) zatrzymuje się na statystykach.
Realna wartość leży w gotowym **planie działań** — wyniku, który mówi właścicielowi CO
zrobić, a nie tylko ile czego wpłynęło. To właśnie krok od podsumowania do planu jest
sednem produktu.

## User & Persona

Primary persona: **Właściciel małej firmy.** Sam odpowiada za zbieranie zgłoszeń, ich
analizę i decyzje usprawniające — nie ma osobnego działu obsługi klienta ani analityka.
Sięga po produkt w momencie, gdy zgłoszeń uzbierało się na tyle, że ręczne przejrzenie
i wyciągnięcie wniosków staje się barierą.

### Secondary persona
Klient firmy — anonimowy autor zgłoszenia. Nie jest użytkownikiem aplikacji w sensie
konta; styka się z produktem wyłącznie przez publiczny formularz zgłoszeniowy. MVP
służy przede wszystkim właścicielowi; klient ma jedną ścieżkę: złożyć zgłoszenie.

## Success Criteria

### Primary
- Właściciel przechodzi pełny przepływ: rejestracja → wprowadzenie danych firmy →
  wygenerowanie adresu formularza zgłoszeniowego oraz kodu QR → zebranie zgłoszeń
  (przez formularz lub dodane ręcznie) → wygenerowanie na żądanie podsumowania
  najczęstszych problemów oraz **planu działań** → zapis planu na koncie. Produkt
  zadziałał, gdy z surowych zgłoszeń powstaje zapisany, sensowny plan działań.

### Secondary
- Wygenerowany plan da się edytować / uzupełnić notatkami przed zapisaniem (miłe, ale
  niewystarczające samo w sobie do uznania MVP za udane).
...
- Żadne wysłane zgłoszenie nie ginie — jest trwale zapisane i dostępne do analizy.
...
- **Given** klient, który otworzył publiczny adres formularza (z identyfikatorem firmy)
  lub zeskanował kod QR
...
- FR-001: Właściciel can zarejestrować się i zalogować do konta firmy. Priority: must-have
  > Socrates: Kontrargument rozważony (auth zbędny dla solo / zawodność dostarczania wiadomości). Rozwiązanie: zostaje — uwierzytelnienie jest niezbędne dla izolacji danych firmy.
...
- FR-004: Właściciel can wygenerować adres formularza zgłoszeń zawierający nieprzewidywalny identyfikator firmy. Priority: must-have
  > Socrates: Kontrargument przyjęty: sekwencyjny identyfikator umożliwia enumerację linków innych firm. Rozwiązanie: identyfikator firmy w publicznym adresie musi być nieprzewidywalny (nie sekwencyjny) — patrz NFR; mechanizm to decyzja downstream.
- FR-005: Właściciel can wygenerować kod QR na podstawie adresu formularza. Priority: must-have
...
- FR-006: Klient can wysłać zgłoszenie przez publiczny formularz bez logowania. Priority: must-have
  > Socrates: Kontrargument rozważony (anonimowość = ryzyko spamu). Rozwiązanie: zostaje — brak tarcia ważniejszy; ryzyko spamu adresowane jako NFR.

### Zgłoszenia
- FR-007: Właściciel can przeglądać zgłoszenia swojej firmy. Priority: must-have
  > Socrates: Kontrargument rozważony (surowa lista przytłacza). Rozwiązanie: zostaje — to analiza robi syntezę, nie ręczne czytanie.
...
- FR-011: Właściciel can na żądanie wygenerować podsumowanie najczęstszych problemów oraz plan działań na podstawie zgłoszeń. Priority: must-have
  > Socrates: Kontrargument przyjęty: wygenerowany plan może opierać się na zmyślonych problemach, co jest gorsze niż brak planu. Rozwiązanie: zostaje (to serce produktu), ale plan musi być wyraźnie oparty na realnych zgłoszeniach (odniesienia/cytaty) — patrz NFR.
...
- FR-014: Właściciel can edytować i usuwać zapisane plany działań. Priority: must-have
  > Socrates: Kontrargument rozważony (edycja zaciera, co dało generowanie). Rozwiązanie: zostaje — właściciel musi dopracować i sprzątać plany.
...
- Podczas generowania planu użytkownik widzi ciągły, widoczny postęp; żadna operacja
  trwająca dłużej niż ~2 s nie sprawia wrażenia zawieszonej.
...
- Publiczny formularz odpiera masowe, automatyczne zgłoszenia (nadużycia), nie blokując
  przy tym zwykłego klienta składającego pojedyncze zgłoszenie.
...
- **Właściciel (uwierzytelniony).** Dostęp do konta firmy wymaga uwierzytelnienia.
  Płaski model: jedno konto = jedna firma, jeden właściciel; brak ról i zapraszania
  zespołu w MVP. Właściciel widzi i zarządza wyłącznie danymi własnej firmy:
  informacjami o firmie, zgłoszeniami oraz wygenerowanymi planami działań.
- **Klient (anonimowy).** Brak konta i logowania. Dostęp wyłącznie do publicznego
  formularza zgłoszeniowego pod adresem zawierającym identyfikator firmy (lub przez
  kod QR). Może jedynie wysłać zgłoszenie do wskazanej firmy; nie ma wglądu w żadne dane.

Nieuwierzytelniony użytkownik próbujący wejść na trasę właściciela jest kierowany do
logowania. (Preferowany bezhasłowy sposób logowania właściciela to decyzja należąca do
etapu doboru technologii — patrz hand-off.)
...
- Import opinii z zewnętrznych platform z recenzjami — brak zewnętrznych źródeł zgłoszeń w MVP.
- Aplikacja mobilna — na początek wyłącznie web.

Niefunkcjonalne / biznesowe:
- System płatności i subskrypcji (warstwa billingowa, plany taryfowe) — produkt na tym
  etapie nie pobiera płatności.

## Open Questions

1. **Minimalna liczba zgłoszeń dla sensownego planu** — czy generowanie planu wymaga
   progu (np. minimalnej liczby zgłoszeń), poniżej którego wynik byłby zbyt ogólnikowy?
   Owner: user. Block: nie (MVP może startować bez progu; do doprecyzowania).
2. **Akceptowalny górny czas generowania planu** — NFR określa widoczny feedback i próg
   ~2 s na brak wrażenia zawieszenia, ale nie ma twardego limitu czasu całej operacji.
   Owner: user. Block: nie.
3. **Próg/akceptacja odporności na masowy spam** — NFR opisuje właściwość (odpieranie
   masowych zgłoszeń bez blokowania klienta); konkretny poziom akceptacji do ustalenia.
   Owner: user, do rozstrzygnięcia na etapie doboru technologii. Block: nie.
